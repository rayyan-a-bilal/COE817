...
def start_client(client_name):
    try:
        ...
        print(f"{client_name}: Received encrypted master key from KDC. Encrypted key: {encrypted_master_key.hex()}")

        master_key = decrypt_with_private_key(private_key, encrypted_master_key)
        print(f"{client_name} decrypted master key: {master_key.decode()}")

        # Here, you'd use the master key for secure communication
    except Exception as e:
        print(f"Error in {client_name}: {e}")


if __name__ == "__main__":
    client_name = "Alice"  # Or "B", depending on the client
